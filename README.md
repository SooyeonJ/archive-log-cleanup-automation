...
아카이브 로그 장기 누적으로 인해  
`/data` 파일시스템 사용률이 약 **52% (773GB)** 수준까지 증가한 상황에서  
하기 목표 달성을 목적으로 함
...
트랜잭션 발생에 따라 아카이브 로그가 지속적으로 생성되는 구조임.

장기간 로그 정리가 수행되지 않아  
`/data` 파일시스템 기준 디스크 사용량이 약 **773GB**,  
사용률이 약 **52%** 수준까지 증가한 상태 확인.

이로 인해 다음과 같은 문제 발생 가능성 존재.
...
아카이브 로그 정리 작업은 **매일 07:30 실행**되며,  
실행 시점 기준 **최근 2주(14일)를 초과한 로그만 삭제 대상**으로 판단함

---

## 정량적 성과 (로그 기반)

아카이브 로그 정리 자동화 작업 수행 결과,  
디스크 사용량 감소 및 운영 안정성 개선 효과 확인.

### 디스크 사용량 변화

```text
[작업 전]
/data 파일시스템 사용량 약 773GB
사용률 약 52%

[작업 후]
/data 파일시스템 사용량 약 108GB
사용률 약 8%

```bash
30 7 * * * /path/to/archive_cleanup.sh >> /path/to/archive_cleanup_cron.log 2>&1
